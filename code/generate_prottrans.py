...
# seq_path = "../../../data/sequences/ara_and_eff.fasta"
...
def split_seq(seq_file, temp_dir):
    os.makedirs(f"{temp_dir}",exist_ok=True)
    seqs = {i.id:str(i.seq) for i in SeqIO.parse(seq_file,"fasta")}

    with open(os.path.join(temp_dir,"input_for_prottrans.fasta"),"w") as f:
...
import pickle
import numpy as np
def extract_prottrans_feature(result_emb):
    #id_dict to represent long and sort sequences.
    ids_embs = dict()
    for ID in result_emb.keys():
        if "__" in ID:
            k = ID.split("__")[0]
            if k not in ids_embs.keys():
                ids_embs[k]=[ID,]
            else:
                ids_embs[k].append(ID)
        else:
            ids_embs[ID] = [ID]
    ...
    print("----------")
    #print(ids_embs.items())
    prottrans = dict()
    for idx,(k, IDs) in enumerate(ids_embs.items()):
        print(k,IDs)
        if len(IDs) == 1:
            prottrans[k] = result_emb[IDs[0]]
            #print(f"{idx} {len(ids_embs)} {k}")
        else:
            temps = [result_emb[ID] for ID in IDs ]
            weights = np.array([ID.split("__")[1].split("_")[1] for ID in IDs],np.float32)
            weights = weights / weights.sum()
            prottrans[k] = np.array([t*w for t,w in zip(temps,weights)]).sum(0)
            #print(f"{idx} {len(ids_embs)} {k}")
    return prottrans
...
def main():    
    ...
    #split sequences
    split_seq(args.input, args.tmp_dir)
    ...
    seqs = read_fasta( os.path.join(args.tmp_dir,"input_for_prottrans.fasta") )

    # Compute embeddings and/or secondary structure predictions
    results = get_embeddings( model, tokenizer, seqs,
                            per_residue=False, per_protein=True, sec_struct=False)
    #print(results)
    # Store per-residue embeddings
    #if per_residue:
    #  save_embeddings(results["residue_embs"], per_residue_path)
    import pickle
    prottrans_emb = extract_prottrans_feature(results['protein_embs'])
    with open(args.output,"wb") as f:
        pickle.dump(prottrans_emb,f)
main()
