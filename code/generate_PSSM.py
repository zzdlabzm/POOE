...
input_seqs_fname = "input.fasta"         # input fasta 
input_dir = "./query/"             # one protein one fasta file!
output_psiblast_dir = "./psiblast" # psi-blast process detaill
output_pssm_dir = "./pssm"         # psi-blast final output pssm matrix
output_pssm_pickle_path="./feature_pssm.pkl"
...
seqs = {_.id:str(_.seq) for _ in SeqIO.parse(input_seqs_fname,"fasta")}
for prot_id,prot_seq in seqs:
...
for idx,file_name in enumerate(query_files):
    ...
    if prot_id not in done_list:
        ...
        print(f"{idx}/{len(query_files)} {cmd}")
        os.system(cmd)
